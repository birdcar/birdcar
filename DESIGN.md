...
description: "Marketing world built first on the homepage and now shipped site-wide as shared chrome: pale daylight ground, ink, canary yellow for actions and packets, a teal fixed-state accent, Mona Sans grotesk headings and body, one Alkaline wordmark, and (on the homepage) a canvas-drawn packet-flow diorama. The Walkthrough and Work pages have also migrated: Walkthrough with its own desk-plate composition and a fit-check card gating the calendar, Work with a scroll journey of three isometric rooms joined by a canary dotted route. The clear argument (cyan fields, Barlow, flat diagrams) is retired as direction and still renders inside the tools and Writing page bodies until each migrates; Admin keeps its Flux Pro/Inter system unchanged."
colors:
...
**Legacy Marketing Font:** Barlow is still self-hosted via `bunny()` and still named by the retired page-body rules (`@layer components` in `resources/css/marketing.css`) — non-heading text, captions, essay meta — on the tools and Writing pages. Because those rules sit inside a CSS layer and the site-wide heading rule does not, Barlow no longer reaches any h1/h2/h3, even on unmigrated pages.
...
- **Don't** describe `/tools/where-work-gets-stuck` or the Writing page bodies as migrated to the new world; their page sections still render The clear argument (cyan fields, Barlow non-heading text, legacy layouts) in `@layer components` in `resources/css/marketing.css` until each migrates one by one.
