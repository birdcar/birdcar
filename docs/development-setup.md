...
- **HTTP 429 rate limit, or a reply the app rejects:** the activity is marked failed. The application never retries failed work automatically; after the limit clears or the cause is fixed, choose **Try again** on the failed activity in the workspace. The run rechecks ownership and frozen inputs first, and after the retry limit a further failure pauses the activity for review.
