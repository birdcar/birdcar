...
**Post-commit corrective review**: `cb56dcc51b6083ee2c73e432604fe8278734edb0` passed the engine, but an independent final audit found `normalizeScheduleIntent()` still silently supplies UTC when callers pass `$scheduledAt` without `selected_timezone` / `scheduled_wall_time`. Close that domain-boundary bypass: require explicit scheduling intent for every scheduled package, validate it against the supplied instant, and reject missing intent without persisting a package or mutating approvals/live pointers. Update legitimate scheduled-call fixtures to supply deliberate matching intent; do not weaken their existing assertions. The workspace already supplies explicit intent. This corrective rerun is intentional despite the earlier phase commit; preserve that commit and every receipt. No new feature or operational authority is added.
