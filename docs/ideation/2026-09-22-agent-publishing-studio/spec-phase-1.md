...
**Retry context**: `run-2026-09-22.json` records the initial strict run's two unresolved review findings. Continue from the existing uncommitted implementation; do not discard or rewrite it wholesale. Add failing regressions for both findings below, fix them, and rerun the complete phase validation/review. The owner authorized this retry and tested/reviewed phase commits without further message prompts.
...
A saved idea has no running attempt. Develop creates the attempt and makes interview/brief work eligible; Phase 4 dispatches it. Within Developing, angle approval permits research/challenge/outline/visual planning; plan approval permits full drafting. The state proceeds through Drafting, InReview, Approved, Scheduled and Published. A separate pause/park/abandon marker retains where work stopped. Missing answers or blockers pause; resume does not imply answering them. Published refers to an attempt's completed release, not to absence of a newer working attempt. Develop may reuse an existing nonterminal attempt, including a paused/parked one without implicitly resuming it. It must never reuse a Published or abandoned attempt. A deliberate Develop after publication creates a new attempt and allowance, requires fresh approvals, and preserves the previous live release and manuscript. Regression: publish a release, call Develop, assert a different attempt ID and unchanged live pointer; a duplicate Develop click then returns that new attempt without another allowance.
...
Both release approval and delivery must reject a package whose revision is no longer the article's current working revision; withdrawing scheduled packages alone is insufficient. Regression: prepare and approve an unscheduled package, save a newer manuscript, then try to re-approve and deliver the old package. Both operations fail without changing the live pointer or restoring approval; preparing and approving a fresh package from the new revision succeeds. Keep these checks inside the locked transaction. Any delivery operation rechecks the exact approval, current version and unresolved blockers inside the transaction. Use the expected previous live release as a compare-and-swap condition so an older scheduled job cannot replace a more recent publication. Duplicate delivery of the same package is a no-op with the same result; mismatched versions fail closed. Preserve the first publication date on revisions. Public readers will later use only `published_release_id`.
